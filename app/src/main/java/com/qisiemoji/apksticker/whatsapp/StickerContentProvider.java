...
import com.qisiemoji.apksticker.util.LogUtil;
...
public class StickerContentProvider extends ContentProvider {
    ...
    public static final String STICKERS_TEMP_FILE = "stickers_tmp_asset";
    ...
    public static final String STICKERS_FILE = "stickers_asset";
    ...
    public static void addStickerPackToMatcher(StickerPack stickerPack) {
        if (!WAStickerManager.getInstance().isCreateStickerPackEnable()) {
            return;
        }
    ...
    public AssetFileDescriptor openAssetFile(@NonNull Uri uri, @NonNull String mode) {
        if(BuildConfig.IS_CONTAINS_ASSET){
            ...
            LogUtil.i("xth", "StickerContentProvider>>openAssetFile() uri=" + uri.toString() + ";mode=" + mode);
    ...
    public ParcelFileDescriptor openFile(@NonNull Uri uri, @NonNull String mode) throws FileNotFoundException {
        final int matchCode = MATCHER.match(uri);
        LogUtil.i("xth","StickerContentProvider openFile()="+uri.toString()+";mode="+mode);
        if (matchCode == STICKERS_ASSET_CODE || matchCode == STICKER_PACK_TRAY_ICON_CODE) {
    ...
    private synchronized void readContentFile(@NonNull Context context) {
        if(BuildConfig.IS_CONTAINS_ASSET){
            ...
            stickerPackList = StickerPackListActivity.stickerPackList;
        }
    }
    ...
    public List<StickerPack> getStickerPackList() {
        ...
        if (WAStickerManager.getInstance().isCreateStickerPackEnable()) {
            ...
            list.addAll(stickerPackList);
            list.addAll(WAStickerManager.getInstance().getStickerPacks(getContext(),WAStickerManager.FileStickerPackType.Publish));
            return list;
        }
    ...
    private ParcelFileDescriptor getImageFile(Uri uri) throws IllegalArgumentException {
        LogUtil.i("xth","StickerContentProvider>>getImageFile() uri="+uri.toString());
        AssetManager am = getContext().getAssets();
        final List<String> pathSegments = uri.getPathSegments();
        if (pathSegments.size() != 3) {
            throw new IllegalArgumentException("path segments should be 3, uri is: " + uri);
    ...
    private ParcelFileDescriptor fetchParcelFile(@NonNull Uri uri, @NonNull String fileName, @NonNull String identifier) {
        try {
            String path = FileUtils2.getFileDir(getContext(),StickerContentProvider.STICKERS_FILE)
                    + File.separator + identifier + File.separator + fileName;
            LogUtil.i("xth","StickerContentProvider>>fetchParcelFile()="+path);
