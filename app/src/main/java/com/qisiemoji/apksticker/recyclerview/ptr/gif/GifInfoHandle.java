...
final class GifInfoHandle {
    ...
    static protected boolean isLoadLibrarySuc = false;

    static {
        try {
            System.loadLibrary("pl_droidsonroids_gif");
            isLoadLibrarySuc = true;
        } catch (Throwable t) {
            t.printStackTrace();
        }
    }
    ...
    synchronized void recycle() {
        // java.util.concurrent.TimeoutException: pl.droidsonroids.gif.GifInfoHandle.finalize() timed out after 10 seconds
        try {
            free(gifInfoPtr);
        } catch (Exception e) {
            e.printStackTrace();
